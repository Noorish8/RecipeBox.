...
public class FavouriteFragment extends Fragment {
    ...
    public View onCreateView(@NonNull LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
        ...
        if (JsonUtils.isNetworkAvailable(requireActivity())) {
            new getFav(API.toBase64(jsObj.toString())).execute(Constant.API_URL);
        }
    ...
    private class getFav extends AsyncTask<String, Void, String> {

        String base64;

        private getFav(String base64) {
            this.base64 = base64;
        }
        ...
        for (int i = 0; i < mListItem.size(); i++) {
            if (mListItem.get(i) != null) {
                if (mListItem.get(i).getRecipeId().equals(saveJob.getReId())) {
                    ...
                    favoriteAdapter.notifyItemRangeChanged(i, mListItem.size());
                 }
            }
        }
    }
